...
- [ ] **T23 — Calendar read + tentative block writer (GATED on T19)** (M) — do not start until T19's credentials exist. Read the day's existing events first (hard constraints, never propose over them); write proposed blocks as real tentative calendar events (title-prefix marker, e.g. `〜`) for: fixed 6am workout anchor, 30-min protected minimums for polymath + SWE-learning, then dynamic-priority items from T21+T22 filling 7am–10pm. Verify: typecheck, rebuild, manual smoke against Samy's actual calendar for one day (document the result in the log entry — this one can't be fully verified by an unattended agent alone, flag for a human glance next session if the calendar-write looks wrong).
  ...
  BLOCKED (2026-07-09, autoloop): the greenlight covers the live-calendar *write*, but wiring the OAuth client up requires adding `GOOGLE_CALENDAR_CLIENT_ID`/`_SECRET`/`_REFRESH_TOKEN` (currently only in `~/.config/homelab/secrets.env`) into `app/.env` (same pattern as the Todoist token) so the container's `env_file: .env` picks them up — that's editing a `.env` file unattended, a separate CLAUDE.md law ("never touch ... `.env` files ... unattended") that this greenlight didn't address. Declining to touch `app/.env` autonomously; left unchecked. Recommend Samy either add the three vars to `app/.env` himself, or explicitly extend the greenlight to cover the `.env` edit so a future autoloop run can proceed with the full implementation in one pass.

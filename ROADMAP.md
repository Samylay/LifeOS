...
- [ ] **T17b — NEEDS-SAMY: define a retirement trigger for the Telegram dual-write** (S) — flagged 2026-07-09: every alert currently fans out to Telegram + ntfy + /pager permanently, because the pager can't report its own death and Telegram is documented as delivery-of-record indefinitely. That's fine as a bridge but has no exit condition today. Decide a concrete trigger (e.g. "a `pager-up` standing goal exists and holds green for N days") and log it here; once met, an executor task can drop Telegram from the fan-out for non-critical streams (keep it for `alerts`/`page` severity regardless, per the homelab CLAUDE.md law that critical pages must never become pager-only).
...
- [ ] **T18b — NEEDS-SAMY: native Android app via Capacitor, unblocked alternative to T18** (M) — flagged 2026-07-09: T18's RN/Expo path is stuck on Firebase (FCM) + Expo/EAS accounts. Capacitor wraps the *existing* live LifeOS UI in a real installable APK (WebView-backed, not a browser tab) with no new paid accounts — push stays exactly as-is today (the ntfy app), so the whole FCM blocker disappears. Two decisions before an executor can build this:
  1. **Transport: use the Cloudflare Tunnel domain (`lab.samylayaida.com`), not the tailnet IP.** Same session flagged that the tailnet approach (raw IP `100.124.149.101:3000`, cleartext HTTP, Android network-security-config exception) is hitting real limits — Samy's phone runs GrapheneOS with Tailscale DNS off (carrier eSIM issue), so it can't resolve `.ts.net` names and needs the IP hardcoded. The tunnel domain is normal public HTTPS/DNS, works on any network, and removes the cleartext-traffic config entirely. LifeOS itself still has no app-level auth (single hardcoded local user) — the only gate today is Cloudflare Access's email-OTP browser flow, so the app needs one of: (a) drive the OTP flow once in an embedded WebView and rely on the resulting cookie's TTL, or (b) a scoped Cloudflare Access Service Token for a non-interactive path. **Samy: pick (a) or (b).**
  2. **Approve the new dependency:** `@capacitor/core` + `@capacitor/android` (two npm packages) — per the homelab CLAUDE.md law, no dependency gets added unattended.
  Build the APK via **GitHub Actions**, not on quorky — no Java/Gradle/Android SDK exist on quorky today and the disk is already at ~79%; GitHub's runners ship Android SDK preinstalled. Once both decisions are made: split into executor tasks (capacitor init pointed at the tunnel domain, Access-auth handling per the chosen option, CI workflow to build + attach the APK as a release artifact, sideload-test on Samy's phone).
...
- [ ] **T33 — Motion foundation** (S) — add easing/duration CSS custom properties to `app/src/app/globals.css` (`--ease-out-custom: cubic-bezier(0.25,1,0.5,1)`, `--ease-in-out-custom: cubic-bezier(0.45,0,0.15,1)`, `--ease-drawer: cubic-bezier(0.32,0.72,0,1)`, `--dur-fast:150ms`, `--dur-base:200ms`, `--dur-slow:300ms`) plus the `prefers-reduced-motion` reduce block from the interaction-craft skill. No component changes yet. Verify: typecheck, rebuild, redeploy, `/` 200; grep confirms vars + media query present.
- [ ] **T34 — Tactility pass** (M) — press feedback (`active:scale-[0.97] transition-transform duration-150` or equivalent) on all buttons, checkboxes, nav items, and actionable cards across `app/src/components`. Replace every `transition-all` with property-specific transitions. Make task check-off optimistic if it isn't already (state flips instantly, server catches up). Keyboard-driven flows stay instant. Verify: typecheck, tests green, rebuild, redeploy, smoke /, /tasks, /prime, /pager all 200; `grep -rn 'transition-all' app/src` empty.
- [ ] **T35 — Enter animations** (M) — page sections (brief, status, tasks, areas) fade+slide in per navigation (`opacity 0→1`, `translateY(4px)→0`, 200ms `--ease-out-custom`, stagger ~30ms via animation-delay). Dropdowns/menus: origin-aware scale from 0.95, 150ms. Hover lift on cards (translateY(-2px) + shadow, 200ms). CSS only — no new deps. Verify: typecheck, tests, rebuild, redeploy, smoke all touched routes; animated properties are transform/opacity only (grep).
- [ ] **T36 — Sonner toasts** (S) — dependency `sonner` PRE-APPROVED by Samy 2026-07-10. Add `<Toaster position="bottom-right" />` to the root layout and replace inline "saved ✓"/alert feedback patterns with `toast()` calls. Verify: typecheck, tests, rebuild, redeploy, a save action shows a toast, / 200.
- [ ] **T37 — Vaul mobile drawers** (M) — dependency `vaul` PRE-APPROVED by Samy 2026-07-10. Use Vaul drawers for mobile detail/quick-add surfaces (start with the highest-traffic one, e.g. task detail or quick-add) with default iOS curve + velocity dismissal; desktop keeps existing UI (media-query or `useIsMobile` split). Add `navigator.vibrate?.(10)` on task completion. Verify: typecheck, tests, rebuild, redeploy, drawer opens/dismisses on a narrow viewport (playwright or manual note), desktop unchanged, touched routes 200.
- [ ] **T38 — Delight + stats polish** (M) — celebration animation (SVG stroke draw-in or scale spring, ≤400ms, transform/opacity only) on rare meaningful events: goal shipped, streak milestone, prime completion. Count-up numbers on /status and /workouts headline stat tiles; staggered tile load-in + ~400ms ease-out chart mount on /workouts (absorbs the retired strava-dashboard polish plan). Respect reduced-motion (skip count-ups/celebrations). Verify: typecheck, tests, rebuild, redeploy, /status /workouts 200.

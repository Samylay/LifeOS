...
- [ ] **T43 — NEEDS-SAMY (decide on 2026-07-14): keep nightly adaptive-spec generation, or revert to lazy?** (S) — the adaptive-UI experiment (Samy's 2026-07-12 ruling) generates a view spec for EVERY approved triage item during the nightly grabbers pass (`~/services/triage/run-grabbers.sh`, adaptive step). Tradeoff: nightly = every approved card opens instantly into its full workspace, at ~1 sonnet call per new approval per night; lazy = zero standing cost, but a card falls back to the thin client-side `fallbackSpec` until a spec exists. Samy asked to be re-asked after 2026-07-13. **Approve = keep nightly** (no change needed). **Reject = revert to lazy**: set `ADAPTIVE_NIGHTLY=0` at the top of `~/services/triage/run-grabbers.sh` (one line; documented there and in `app/src/components/decide/adaptive-prototype/NOTES.md`). Executor: Samy's call only — do not act before a `SAMY:` verdict line exists here.
- [ ] **T44 — NEEDS-SAMY: fix the Cloudflare Access Service Token for the Android app** (S) — found 2026-07-12 while reinstalling the APK (mic-permission rebuild): the Service Token in `~/.config/homelab/secrets.env` is rejected by Access — `curl https://lab.samylayaida.com` with the `CF-Access-Client-Id/Secret` headers returns the same 302-to-login as without them, so the app falls back to in-app OTP on every fresh install (`adb logcat -s CfAccess` shows "Access challenged … 0 retries left"). Fix is dashboard-side (Cloudflare Zero Trust): either the token expired/was rotated (mint a new one and update both secrets.env values), or the Access application for `lab.samylayaida.com` never got a **Service Auth** policy accepting this token (add it — `app/docs/android-app.md` "Server-side state" has the details). Verify: the curl above returns 200 with the headers, then relaunch the app — `adb logcat -s CfAccess` shows no challenge and no OTP page. Executor: Samy's call/dashboard only — do not touch secrets.env unattended.

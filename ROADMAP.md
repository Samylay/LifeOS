...
- [ ] **T60 — Scheduling a topic writes a Todoist task** (M) — GATED on T57. Per map 03. **Scheduling is the commitment act**: `scheduleTopic(topicId, date)` also writes ONE Todoist task (`"Learn: <sentence>"`, due that date); Samy's own Todoist→calendar sync does the rest. Todoist holds **only what he committed to** — never mirror the queue, never create a daily recurring task. **LifeOS currently only READS Todoist** (`brief/fetchers/todoist-centres.ts:6`, `work.ts:11-12`) — a write is a POST to the same `api/v1/tasks` with the token already in the secrets store: **not a new dependency**. Fail soft: a Todoist outage must not lose the schedule (the topic is still `scheduled` locally; log + retry). **The live round-trip is NOT the autoloop's to run** — creating a task in Samy's real Todoist is user data + live service state, which the executor contract forbids. Build it behind `TODOIST_WRITE_ENABLED` (default OFF) so the nightly agent can ship and verify the code without ever touching his account; Samy flips it on and does the first real write himself. Verify (autoloop): tsc, `npx vitest run` green incl. tests for the task body/date shape, the flag defaulting off, and a write failure leaving local state consistent; rebuild, redeploy, `/knowledge` 200; `grep` proves no POST fires with the flag unset. **Then leave a NEEDS-SAMY log note that the first live write + flag flip is his.**
...
> 2. **Direct SG/Revolut PSD2 APIs are a permanent dead end** — they require a licensed AISP + eIDAS certificate. All bank access goes through the aggregator (GoCardless BAD), which holds the licence. Never "try the bank API directly".
...
- [ ] **T67 — NEEDS-SAMY: GoCardless Bank Account Data account + credentials** (S) — gates T68–T71; nothing downstream can be verified against live data until this lands. Samy: (a) sign up at gocardless.com for Bank Account Data (free production tier) — **this also answers Q1 in the map: whether the free tier admits individuals rather than registered businesses**; (b) create a secret ID + secret key; (c) put them in `app/.env` as `GOCARDLESS_SECRET_ID` / `GOCARDLESS_SECRET_KEY` (gitignored; `env_file: .env` already wires it into the container — no docker-compose.yml change needed, so no infra commit); (d) confirm SG and Revolut both resolve in their institution list for FR. If the free tier rejects individuals, record that in the map and pick an alternative from the candidates listed there (Tink/TrueLayer/Yapily/Salt Edge/Plaid) — do not proceed on a guess. Verify: `docker compose exec app printenv GOCARDLESS_SECRET_ID` is non-empty (**print the var name only, never the value**).

- [ ] **T68 — GoCardless client wrapper, offline-testable** (M) — `src/lib/gocardless.ts`, server-side only. Follow the `src/lib/google-calendar.ts` precedent exactly: read creds from `process.env`, expose an `isConfigured()` guard so every call path degrades cleanly when creds are absent (they will be, until T67). Cover: token exchange (access/refresh), list institutions (FR), build a requisition + return the bank consent link, fetch accounts, fetch transactions. **No network in tests** — inject the transport and test against recorded, *redacted* fixtures (rule 1: scrub account ids, IBANs, amounts, names). This task needs no credentials and is safe to do before T67. Verify: `cd app && npx tsc --noEmit && npx vitest run src/lib/gocardless.test.ts` green, and `grep -rn "GOCARDLESS" src/ --include=*.test.ts` shows no real secret.

- [ ] **T69 — Transactions schema + sync** (M) — additive only: new `CREATE TABLE IF NOT EXISTS` in the `src/lib/server-db.ts` style (`accounts`, `transactions`). **Do not alter or migrate any existing table** (constitution: no unattended migrations). Transactions dedup on the aggregator's transaction id — a re-sync must be idempotent, since 24 months of history gets pulled on first connect and re-pulled on every re-consent. Store the raw payload alongside normalised fields; the detector (T70) will want fields we haven't thought of yet. Sync is **manual-trigger only** in this task (an API route), not scheduled — scheduling waits until the shape is proven. Verify: `npx tsc --noEmit`; vitest covers a sync against fixtures run **twice**, asserting the second run inserts 0 rows and mutates nothing.

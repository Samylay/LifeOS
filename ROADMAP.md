...
- [ ] **T53 — Learning model: topics + tags (schema + store)** (M) — foundation for T54–T64; no UI. Per map tickets 01/02/12. `TeachTopic` becomes a **container**: `topic` is a sentence, `mission` REQUIRED (non-empty), `tags: string[]` (the controlled topic-tags it owns), `origin: "authored" | "proposed"`, plus existing `status`/`scheduledFor`/`learningRecords`. **Delete `adoptSuggestion()` and `teachSuggestions()` outright** (map 01/05 — inverse of the decided model; `teachSuggestions`' hardcoded `["ai-tip","ai-project","swe"]` filter has NOTHING replacing it) and the single-string `origin` shape they implied. Add the controlled tag list as its own collection (`users/local/topicTags`: `{tag, neverPropose: boolean}`) — `neverPropose` is ticket 11's tombstone, nothing works without it. `TriageItem` gains `topicTags: string[]` (controlled) + `vaultTags: string[]` (free-form); `category` **SURVIVES UNCHANGED** as its own axis (map 02 — it drives /decide assessments + triage destinations, which are not learning concerns; do NOT collapse it into tags). Existing docs lack the new fields — read them tolerantly (absent = empty), **do not migrate the live DB** (that's Samy's, see T64c). Verify: `npx tsc --noEmit`, `npx vitest run` green incl. new unit tests (a topic without a mission is rejected; tolerant read of a legacy doc; tombstone round-trip), `docker compose build`.
- [ ] **T54 — study.py emits both tag sets** (M) — GATED on T53. Per map 02. `~/services/triage/study.py` already fetches + reads each item to write `{summary, why_relevant, destination, confidence, rationale}` — extend the same call to also emit **`topicTags`** (chosen ONLY from the controlled list, passed into the prompt) and **`vaultTags`** (free-form, for Obsidian's graph). When nothing in the controlled list fits, the model **proposes** a new tag rather than inventing one — write proposals to a pending store for T58's deck; **never self-approve**. Tag both layers when applicable (`[devops, kubernetes]`) — the list is FLAT, there is no hierarchy to walk (map 12). Pass the item's `folder` hint into the prompt as a **signal** (it is four years of Samy's own filing) — it is an input, not a stored field. **This also closes the gap `~/services/hermes/hermes.py:57-64` explicitly deferred to study.py** ("the fix is to have study.py emit tags at file time"): `vaultTags` are what Hermes stopped writing. Verify: dry-run over ≥5 real queued items produces both sets with every `topicTags` entry present in the controlled list; an item with no fitting tag produces a proposal, not a new tag; re-run is idempotent; `grep` confirms no self-approval path.
- [ ] **T55 — Backfill tags over existing proposals** (S) — GATED on T54. Per map 09. One-off, off-repo like `backfill-firefox.py`. **62 of the 67 library items (and 144 of 149 total) already carry full Opus-read proposals — do NOT re-fetch content.** Tag from the existing `summary` + `why_relevant` + `rationale` text. Without this the 4-year library overlaps no topic and never surfaces, which would make the whole "library is supply" ruling dead letter. Expect it to light up ~8-10 clusters at once — **that is intended** (see T58). Verify: run over the live queue, every item ends with a `topicTags` array, spot-check 10 items by hand against their summaries, count of items with ≥1 tag ≈ count with a proposal.
...
- [ ] **T58 — Proposals: tag + topic cards on /decide** (M) — GATED on T54+T56. Per map 11 (+02). **Trigger: count alone — N items share a tag AND no topic owns that tag → propose. NEVER consult age, recency, or density** (rule 1 above; a 2023 cluster and a this-week cluster are equal). **One deck, two card types**, reusing `CardStack` like the triage/NEEDS-SAMY/Shelf tabs: tag cards are tiny ("add tag `x`?"); topic cards ask for the **why** on accept (map 06 — a topic cannot exist without a mission). **"Never" tombstones the tag permanently** (`neverPropose`) — this is what stops `[humor]`×12 proposing "learn humor", and it is the ONLY eligibility mechanism (map 11: the first proposal *is* the eligibility question — do not add a `learnable` field or a model judgement). **Day one bursts (~8-10 clusters from T55) — that is Samy setting up his curriculum from four years of saves, NOT noise. Do not cap the first run.** Steady-state cap ≤3 proposals/week (copy the weekly compost review's cap). Verify: tests — cluster at N proposes; an old cluster proposes identically to a fresh one; "never" tombstones and re-running proposes it 0 times; accept-without-why is refused; first run uncapped, subsequent weeks ≤3. Redeploy, `/decide` 200, both card types render.
...
- [ ] **T63 — NEEDS-SAMY: confirm chat-queued topics become proposals** (S) — derived on the map, never actually put to Samy (map 06). `add_learning_topic` (the chat→homelab tool) survives, but a topic cannot exist without a mission (map 06), and a one-handed phone message won't carry one. Proposed: a chat-queued topic becomes a **proposal** he completes (with its why) on `/knowledge`, reusing T58's acceptance flow — one acceptance path, degrades gracefully. **Confirm with Samy before building**; the alternative is requiring the why in the message itself.
...
- [ ] **T64b — NEEDS-SAMY: `〜` calendar blocks vs Todoist-synced learning tasks** (S) — from map 03. Once T60 lands, a scheduled topic reaches the calendar via Todoist **while** the brief keeps writing 30-min `〜 polymath` / `〜 swe-learning` protected blocks (`brief/centre-inference.ts:16-17`) — **double-booking the same 30 minutes**. Samy called the calendar blocks *"replaceable and not set in stone"*, which is a **hint, not an instruction**; retiring them touches the brief's tentative-block writer, outside the learning map's scope. **Do not delete his calendar blocks unilaterally.** Ask, then execute.
- [ ] **T64c — NEEDS-SAMY: live-DB migration for the learning model** (S) — from map 10's rules. T53 changes the shape of `teachTopics` (and adds fields to `triageQueue` docs). Existing docs are read tolerantly so nothing breaks, but if a real migration is wanted it is **Samy's to run, never the autoloop's** (constitution: never touch user data unattended). Back up first (`~/backups/lifeos-db/`), per the pre-teach-2026-07-12 precedent.
- [ ] **T64d — NEEDS-SAMY: study.py's `destination` scoring is an unexamined ROI lens** (M) — found while resolving map 09; **outside the learning map's destination, raised because it is live today.** `study.py` writes `destination` by scoring each item against *"the nine centres and the content rubric"* — a **usefulness lens**. **37 of the 67 rescued library items are already `discarded` by it.** The bookmark cull's persona bug was fixed (`~/.config/homelab/persona.txt`: the shelf is aspirational, curiosity alone justifies a keep, "never worked through" = a to-do not a corpse) but **this scoring path was never re-examined**, so it is plausibly still mis-filing his aspirational saves nightly. The learning pipeline routes around it (T56: discard blocks filing, not learning) — but filing itself may still be wrong. Decide with Samy: re-judge past discards, fix the prompt going forward, both, or accept it.

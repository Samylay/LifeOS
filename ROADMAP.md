...
- [ ] **T38 — Enroll triage-nightly standing goal (after first 00:30 run)** (S) — the bookmark-triage pipeline (T33-T36) is durable; graduate it per the constitution once `~/services/triage/grabbers.log` has its first real nightly entry (tonight 15:30 UTC), so the predicate passes on enrollment instead of firing a day-one FAIL (the T26 lesson). Add `~/infra/goals/goals/triage-nightly.md` predicate = a "grabbers done" line with an ISO timestamp in the last 25h; on-violation alert (nightly grabber/study cron dead — check grabbers.log). Verify: `~/infra/goals/verify-goals.sh` green.

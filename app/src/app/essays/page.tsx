...
const field = "mt-2 w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm outline-none focus:border-ring focus:ring-2 focus:ring-ring/20";
...
export default function EssaysPage() {
  ...
  return <Page className="max-w-6xl">
    <PageHeader title="Essay workshop" icon={Feather} actions={<EssayWritingGuide />} />
    ...
    {review && <section id="essay-review" className="scroll-mt-20 space-y-5">
      <div className="enter rounded-2xl border border-border bg-card p-5 shadow-card sm:p-7">
        ...
        {review.priorities.length > 0 && <div className="mt-7 grid gap-3 md:grid-cols-3">{review.priorities.map((priority, index) => <article key={`${priority.dimension}-${index}`} className="rounded-xl border border-border bg-background/60 p-4 transition-transform duration-200 ease-[var(--ease-out-custom)] hover:-translate-y-0.5 hover:shadow-pop">
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_20rem]">
        <div className="space-y-3">{review.dimensions.map((dimension, index) => { const meta = DIMENSIONS.find((item) => item.id === dimension.id)!; return <details key={dimension.id} open={dimension.findings.length > 0 && index < 4} className="group enter rounded-xl border border-border bg-card shadow-card [animation-delay:var(--delay)]" style={{ "--delay": `${Math.min(index * 25, 200)}ms` } as React.CSSProperties}>
          <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-4 active:scale-[0.97] transition-transform duration-150 ease-[var(--ease-out-custom)] sm:px-5"><div className="flex min-w-0 items-center gap-3"><span aria-hidden="true" className={`size-2.5 shrink-0 rounded-full border ${statusStyles[dimension.status]}`} /><div className="min-w-0"><h3 className="truncate text-sm font-semibold">{meta.label}</h3><p className="mt-0.5 line-clamp-1 text-xs text-muted-foreground">{dimension.summary}</p></div></div><Badge variant="outline" className={statusStyles[dimension.status]}>{statusLabels[dimension.status]}</Badge></summary>

...
function parseRuling(line:string):ExtractItem|null { const m=line.match(/^- (?:\*\*)?(.+?)(?:\*\*)? — (.*)$/); if(!m)return null; const ses=m[2].match(/ · \[\[([^|\]]+)(?:\|([^\]]+))?\]\]$/); let summary=m[2]; if(ses)summary=summary.replace(ses[0],""); return {title:clean(m[1].replace(/^\*\*|\*\*$/g,"")),state:"needs ruling",summary:clean(summary),session:ses?.[2]||ses?.[1]};}

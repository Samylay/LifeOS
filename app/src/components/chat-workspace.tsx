...
import { ArrowUp, BookOpen, Check, LoaderCircle, Plus, RotateCcw, Sparkles, Square } from "lucide-react";
    ...
    <section className="mx-auto flex min-h-[calc(100dvh-10rem)] w-full max-w-4xl flex-col">
      <header className="flex h-12 shrink-0 items-center justify-between border-b border-border/70">
        ...
        <button
          ...
          className="inline-flex h-8 items-center gap-2 rounded-lg px-2.5 text-xs text-muted-foreground transition-transform duration-150 hover:bg-muted hover:text-foreground disabled:opacity-40 active:scale-[0.97]"
        >
            ...
            <div className="mt-8 grid w-full max-w-xl gap-2 sm:grid-cols-2">
              {starters.map(({ icon: Icon, title, prompt }) => (
                <button
                  ...
                  className="group flex min-h-16 items-center gap-3 rounded-xl border border-border/80 bg-card/70 px-4 text-left transition-[transform,background-color,border-color] duration-150 ease-[var(--ease-out-custom)] hover:-translate-y-0.5 hover:border-primary/30 hover:bg-card active:scale-[0.97]"
                >
                  ...
                  <div className={cn("min-w-0 max-w-[88%]", message.role === "user" && "rounded-2xl bg-muted/80 px-4 py-3 sm:max-w-[78%]")}>
                    ...
                    {message.interrupted && (
                      <div className="mt-2 flex items-center gap-3 text-xs text-muted-foreground">
                        ...
                        {message.id === messages[messages.length - 1]?.id && !loading && <button onClick={retryLast} className="inline-flex items-center gap-1 font-medium text-primary transition-transform duration-150 hover:underline active:scale-[0.97]"><RotateCcw size={12} /> Retry</button>}
                      </div>
                    )}
                    {!!message.actions?.length && (
                      <div className="mt-3 flex flex-wrap gap-1.5">
                        {message.actions.map((action, index) => <span key={`${action.tool}-${index}`} className={cn("inline-flex items-center gap-1.5 rounded-md border px-2 py-1 text-xs", action.failed ? "border-destructive/25 text-destructive" : "border-primary/20 bg-primary/5 text-primary")}><Check size={12} />{action.summary}</span>)}
                      </div>
                    )}
                  </div>
                </article>
        ...
        <div className="sticky bottom-0 shrink-0 bg-background pb-2 pt-3">
          <div className="mx-auto max-w-3xl rounded-2xl border border-border bg-card shadow-[0_8px_28px_-16px_rgba(0,0,0,0.5)] focus-within:border-primary/35 focus-within:shadow-[0_10px_32px_-16px_rgba(107,142,121,0.32)]">
            ...
            <div className="flex items-center justify-between px-2 pb-2 pt-1">
              <p className="pl-2 text-[11px] text-muted-foreground">LifeOS can make changes when you ask.</p>
              {loading ? (
                <button type="button" onClick={stop} aria-label="Stop response" className="flex size-9 items-center justify-center rounded-xl bg-foreground text-background transition-transform duration-150 active:scale-[0.97]"><Square size={14} fill="currentColor" /></button>
              ) : (
                <button type="button" onClick={submit} disabled={!input.trim()} aria-label="Send message" className="flex size-9 items-center justify-center rounded-xl bg-primary text-primary-foreground transition-[transform,opacity] duration-150 active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-40"><ArrowUp size={18} /></button>
              )}
            </div>
          </div>

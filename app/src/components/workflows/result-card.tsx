...
export function ResultCard({ run }: { run: WorkflowRun }) {
  ...
  return <article className="overflow-hidden rounded-2xl border border-border bg-card">
    <div className="space-y-4 p-4 sm:p-6">
      ...
      <h2 className="text-xl font-semibold [overflow-wrap:anywhere]">{run.title}</h2>
      {report && <>
        <div className="flex items-start gap-2 text-sm"><span className={report.outcome === "pass" ? "text-success" : "text-muted-foreground"}>{report.outcome === "pass" ? <Check size={18} /> : <CircleAlert size={18} />}</span><div><p className="font-medium capitalize">{report.outcome === "reference" ? "Reference prepared" : report.outcome}</p><p className="mt-1 whitespace-pre-wrap text-muted-foreground">{report.summary}</p></div></div>
        ...
        <ul className="space-y-2 text-sm">{report.findings.map((finding, i) => <li key={i} className="flex gap-2"><span className="mt-2 size-1 shrink-0 rounded-full bg-muted-foreground" /><span className="whitespace-pre-wrap">{finding}</span></li>)}</ul>
        {report.limitations.length > 0 && <section className="rounded-lg border border-warning/30 p-3"><h3 className="text-xs font-medium text-warning">Limits of this result</h3>{report.limitations.map((limitation, i) => <p key={i} className="mt-2 whitespace-pre-wrap text-sm text-muted-foreground">{limitation}</p>)}</section>}
        {report.prepared && <section className="rounded-lg border border-border bg-background p-4"><h3 className="text-sm font-medium">{report.prepared.title}</h3><p className="mt-3 whitespace-pre-wrap break-words text-sm leading-relaxed text-muted-foreground">{report.prepared.body}</p>{report.prepared.recipe && <details className="mt-3"><summary className="cursor-pointer text-xs">Exact ingredients and method to save</summary><ul className="mt-3 space-y-1 text-sm">{report.prepared.recipe.ingredients.map((ingredient, i) => <li key={i}>{ingredient.quantity} {ingredient.name}</li>)}</ul><ol className="step-method mt-3">{report.prepared.recipe.steps.map((step, i) => <li key={i} className="text-sm">{step}</li>)}</ol></details>}</section>}
      ...
      <EvidenceDetails itemId={run.itemId} evidenceRef={run.evidenceRef || undefined} />

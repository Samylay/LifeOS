...
export function Sidebar({ mobile }: { mobile?: boolean }) {
  ...
  const [moreOpen, setMoreOpen] = useState(() => Boolean(mobile));
  // Active secondary routes remain visible without an effect-driven state sync.
  const inMoreGroup = MORE_ITEMS.some((i) => isActive(i.href));
  const isMoreOpen = moreOpen || inMoreGroup;
  ...
  const linkClass = (active: boolean) =>
    cn(
      "group relative z-10 flex min-h-10 items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-[color,background-color,transform] duration-[var(--dur-fast)] ease-[var(--ease-out-custom)] active:scale-[0.97]",
      active
    ...
    <>
      ...
      {mobile && mobileSidebarOpen && (
        <button
          ...
          className="fixed inset-0 z-[39] bg-black/55 backdrop-blur-[2px]"
          onClick={() => setMobileSidebarOpen(false)}
        />
      )}

      <aside
        className="glass-panel fixed left-0 top-0 z-40 flex flex-col overflow-hidden border-y-0 border-l-0"
        style={{
          height: "100dvh",
          width: mobile ? 280 : sidebarExpanded ? 244 : 72,
      ...
      >
        ...
        <div className="flex h-14 shrink-0 items-center justify-between border-b border-border px-3">
          <div className="flex min-w-0 items-center gap-2.5">
            <span className="relative grid size-8 shrink-0 place-items-center overflow-hidden rounded-lg border border-white/10 bg-surface-3 font-display text-sm font-bold text-foreground shadow-card">
              L
              <span className="absolute inset-x-0 bottom-0 h-px bg-gradient-to-r from-primary to-[#8b5cf6]" />
            </span>
            {expanded && (
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold tracking-tight text-foreground">LifeOS</p>
                <p className="font-mono text-[9px] uppercase tracking-[0.16em] text-muted-foreground">Mission desk</p>
              </div>
            )}
          </div>
          {mobile && (
        ...
        {/* Nav items */}
        <nav
          className="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto px-2 py-4"
        ...
        >
          ...
          <button
            ...
            className={cn(
              linkClass(false),
              "mt-2 w-full border border-primary/10 bg-secondary/60 text-left text-primary hover:bg-secondary"
            )}
          ...
          <div className="mt-2">
            ...
            >
              <ChevronDown
                ...
                style={{ transform: isMoreOpen ? undefined : "rotate(-90deg)" }}
              />

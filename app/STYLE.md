...
## Mission desk surface system (2026-08-26)

LifeOS uses a stable shell around a dense working canvas. The page is quiet,
the active task is obvious, and support information stays visually secondary.
Tokens live in `src/app/globals.css`.

- **Surface ladder**: Void `#080A10` is the page ground, Deck `#10131D` is the
  working surface, and Lift `#171B28` is reserved for raised menus and dialogs.
  Ordinary cards are opaque. Glass belongs only to shell chrome and overlays.
- **Horizon signature**: a thin cyan-to-violet rule connects active navigation
  to every page header. It provides identity and wayfinding without an animated
  wallpaper or decorative card effects.
- **Elevation**: low-alpha borders define working regions. `--shadow-card` is a
  restrained inner highlight and ambient shadow; `--shadow-pop` is only for
  overlays. Elevation does not animate.
- **Radii**: `--radius-card: 0.875rem` for working surfaces; inputs and buttons
  use the smaller shared radius. Avoid nesting several rounded containers.
- **Page grammar**: `<Page>` establishes width and rhythm, `<PageHeader>` owns
  identity and one action slot, `<FilterBar>` holds local view choices, and
  `.work-canvas` marks the dominant task region. Focused sessions may replace
  this grammar with a full-height task surface.
- **Section labels**: `.section-label` is the one compact uppercase label style.
...
## Identity: Horizon on charcoal (2026-08-26)

The owner-selected Aurora palette remains, but the generic drifting mesh and
glass-card treatment do not. LifeOS spends cyan on action, focus, and live
state; violet supports the horizon signature and data series. Space Grotesk
provides page identity, Inter handles dense UI, and JetBrains Mono is reserved
for telemetry and tabular values. A static low-alpha glow at the shell edge is
the maximum ambient decoration.

## Tokens (`src/app/globals.css`)

- **Semantic layer** — `--background`, `--foreground`, `--card`, `--primary`
  (signal cyan), `--secondary`, `--muted`, `--accent-ui`, `--destructive`, `--success`
...
- **`contrast-color()` is evaluated and shelved** (2026-07-31, prototype in
  ...
  foregrounds.
- Raw sage/warm values remain for historical data mappings, but new interface
  work consumes only semantic mission-desk tokens. Dark-only since 2026-08:
  `.dark` is server-rendered on `<html>` and the dark values are authoritative.
...
- **Typeface roles (2026-08-26)**: `--font-display` = Space Grotesk for page and
  section identity, `--font-sans` = Inter for UI and body, and `--font-mono` =
  JetBrains Mono for code, IDs, and tabular data. Do not choose fonts per route.
...
- **Card shell**: `<Card>` from `ui/card.tsx` is an opaque Deck surface with a
  low-alpha edge and compact padding. Use it for grouped content, not for every
  heading, metric, or row. Prefer one clear work canvas over a grid of equal
  cards.
